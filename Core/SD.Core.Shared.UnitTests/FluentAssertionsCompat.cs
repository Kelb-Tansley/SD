...
namespace SD.Core.Shared.UnitTests;
...
public sealed class AssertionBuilder<T>(T? value)
{
    ...
    private static bool AreEquivalent(object? expected, object? actual)
    {
        ...
        if (IsNumeric(expected) && IsNumeric(actual))
        {
            ...
            if (double.IsPositiveInfinity(expectedValue) && double.IsPositiveInfinity(actualValue))
            {
                return true;
            }

            if (double.IsNegativeInfinity(expectedValue) && double.IsNegativeInfinity(actualValue))
            {
                return true;
            }
