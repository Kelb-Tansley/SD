...
[Binding]
public sealed class BendingOnlyStepDefinitions
{
    ...
    public BendingOnlyStepDefinitions(
    ...
    [Then("the bending resistance of beam (.*) should be (.*)")]
    public void ThenTheBendingResistanceShouldBe(int beamNumber, double bendingResistance)
    {
        var sansUlsResult = _ulsDesignResults.SansUlsResults.FirstOrDefault(sur => sur.Beam.Number == beamNumber);
        sansUlsResult.Capacity.MrMajor.Should().Be(bendingResistance);
    }
}
